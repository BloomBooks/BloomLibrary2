...
You will need to get local language files with `pnpm crowdin-download` to test locally. This requires setting a `BLOOM_CROWDIN_TOKEN` environment variable. This token isn't visible on the Crowdin site; it only shows it once. So you have to get it from another Bloom developer or from the Azure configuration.