    ...
    const handleSearch = () => {
        ...
        if (specialSearchResults.length > 0) {
            ...
            if (specialSearchResults[0] === noPushCode) {
                // check for uiLang override
                if (specialSearchResults.length > 1) {
                    setLanguageOverride(specialSearchResults[1]);
                }
            } else {
        ...
        if (tryHandleLanguageSearch(searchStringLower)) return;
        if (tryHandleFeatureSearch(searchStringLower)) return;
