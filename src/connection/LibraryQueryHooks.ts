...
import { GetSanitizedLanguagesList } from "../model/Language";
...
export function useGetSanitizedLanguagesList() {
    const axiosResult = useGetLanguagesList();
    if (axiosResult.response?.data?.results) {
        axiosResult.response.data.results = GetSanitizedLanguagesList(
