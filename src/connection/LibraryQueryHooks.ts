...
export function useGetBookCount(
    filter: IFilter,
    ...
    if (doNotActuallyRunQuery) {
        //console.log("Did not run query for useGetBookCount");
        return -1;
    }
    if (answer.loading && !answer.response) {
        return -1; // invalid count indicates still loading
    }
    if (!answer.response) {
        return 0;
    }
...
// the axios calls here (in the useAsync(=>retrieveBook(Data|Stats) calls) is shared with getAllGridDataAndExportCsv
